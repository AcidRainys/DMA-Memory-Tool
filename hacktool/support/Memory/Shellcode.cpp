...
uint64_t c_shellcode::find_codecave(size_t function_size, const std::string& process_name, const std::string& module)
{
	...
	{
		LOG_ERROR("[!] Could not retrieve process for PID: %i", pid);
		return 0;
	}
	...
	{
		LOG_ERROR("[!] Could not retrieve sections #1 for '%s'\n", module.c_str());
		return 0;
	}
	...
	{
		LOG_ERROR("[!] Could not retrieve sections #2 for '%s'\n", module.c_str());
		return 0;
	}
	...
	if (!codecave)
	{
		LOG_ERROR("[!] Could not find a code cave for '%s'\n", module.c_str());
		return 0;
	}
	...
	{
		LOG_ERROR("[!] Could not read codecave for '%s'\n", module.c_str());
		return 0;
	}
	...
	for (size_t i = 0; i < function_size; i++)
	{
		if (buffer[i] != 0x0)
		{
			LOG_ERROR("[!] Codecave isn't big enough for the shellcode.\n");
			return 0;
		}
	}
...
bool c_shellcode::call_function(void* hook, void* function, const std::string& process_name)
{
	...
	*reinterpret_cast<uint64_t*>(jmp_bytes + 6) = reinterpret_cast<uintptr_t>(hook);
	...
	{
		LOG_ERROR("[!] 1 Failed to write memory at 0x%p\n", function);
		return false;
	}
	...
	{
		LOG_ERROR("[!] 2 Failed to write memory at 0x%p\n", function);
		return false;
	}
