...
bool c_keys::InitKeyboard()
{
	...
	else
	{
		...
		if (gafAsyncKeyStateExport < 0x7FFFFFFFFFFF)
		{
			...
			{
				LOG_ERROR("failed to find gafAsyncKeyState\n");
				return false;
			}
			LOG_INFO("found gafAsyncKeyState at: 0x%p\n", gafAsyncKeyState);
		}
		if (gafAsyncKeyStateExport > 0x7FFFFFFFFFFF)
			return true;
		return false;
...
bool c_keys::IsKeyDown(uint32_t virtual_key_code)
{
	if (gafAsyncKeyStateExport < 0x7FFFFFFFFFFF)
		return false;
	if (std::chrono::system_clock::now() - start > std::chrono::milliseconds(10))
	{
		UpdateKeys();
